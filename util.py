...
def image_mean_std(dirs):
    print(dirs)
    sub_dirs = os.listdir(dirs)

    sum_img = np.zeros([2048, 2048, 3])
    num_imgs = len(sub_dirs) * 10
    n_idx = 1
    for sub_dir in sub_dirs:
        sub_dir_path = os.path.join(dirs, sub_dir).replace("\\", "/")
        samples = os.listdir(sub_dir_path)

        n_samples = np.random.choice(samples, size=10)

        for sample in n_samples:
            sample_file = os.path.join(sub_dir_path, sample).replace("\\", "/")
            print(str(n_idx) + '/' + str(num_imgs) + ', ' + sample_file)
